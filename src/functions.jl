...
function createFeatures(dataFolder::String, dataSet::String)
    ...
    if !isfile(trainDataPath) || !isfile(testDataPath)
        ...
        if dataSet == "kidney"
            ...
            # Add columns related to the class
            features.class = ifelse.(rawData.class .== "ckd", 1, 0)
            
            # Add columns related to the al									# Q ouverture
            #createColumns(:al, [0, 1, Inf], rawData, features) 
                     
            # Add columns related to the sc									# Q ouverture
            #createColumns(:sc, [0, 0.8, 1.1, 2.2, Inf], rawData, features)  
            
            # Add columns related to the rbcc								# Q ouverture
            #createColumns(:rbcc, [0, 4, 4.8, 5.5, Inf], rawData, features) 
            
            # Add columns related to the htn								# Q ouverture
            features.htn = ifelse.(rawData.htn .== "yes", 1, 0)             
            
            # Add columns related to the dm                            		# Q ouverture
            features.dm = ifelse.(rawData.dm .== "yes", 1, 0)               
			
			# Add columns related to the hemo
            createColumns(:hemo, [0, 11.3, 13.9, 15.5, Inf], rawData, features)
            
			# Add columns related to the pcv
            createColumns(:pcv, [0, 34, 42, 48, Inf], rawData, features)
            
			# Add columns related to the sg
            createColumns(:sg, [0, 1.015, 1.02, 1.025, Inf], rawData, features)
            
			# Add columns related to the pc
        ...
        if dataSet == "housevotes"
            ...
            # Add columns related to the class
            features.class = ifelse.(rawData.class .== "republican", 1, 0)
            
			# Add columns related to the physician
            features.physician = ifelse.(rawData.physician .== "y", 1, 0)
  
            # Add columns related to the adoption
            features.adoption = ifelse.(rawData.adoption .== "y", 1, 0)
            
            # Add columns related to the salvador
            features.salvador = ifelse.(rawData.salvador .== "y", 1, 0)
            
			# Add columns related to the education
            features.education = ifelse.(rawData.education .== "y", 1, 0)

			# Add columns related to the nicaraguan
            features.nicaraguan = ifelse.(rawData.nicaraguan .== "y", 1, 0)
...
function createRules(dataSet::String, resultsFolder::String, train::DataFrames.DataFrame)
    ...
    if !isfile(rulesPath)
        ...
        for y = 0:1
            ...
            # Si j appartient b et j n'appartient pas ti , alors b ne s’applique pas à ti
            @constraint(m, [i = 1:n, j = 1:d], x[i] <= 1+ (t[i,j]-1)*b[j] )

            # Si pour tout j appartient b j appartient ti , alors b s’applique à ti
            @constraint(m, [i = 1:n], x[i] >= 1+ sum((t[i,j]-1)*b[j] for j=1:d) )

            # La borne sur la couverture est respectée
            @constraint(m, [i = 1:n], sum(x[i] for i=1:n) <= cmax )
            ...
            while cmax >= n*mincovy
                if iter == 1 
                    ...
                    bstar = trunc.(Int, JuMP.value.(b))
                    x1 = sum(JuMP.value.(x))
                    iter += 1

                end
                
                rule = convert(DataFrame, hcat(append!([y], bstar)...))   
                ...
                if iter < iterlim
                    ...
                    # La contraintes *
                    @constraint(m, ( sum(b[j] for j=1:d if bstar[j]==0) + sum(1 - b[j] for j=1:d if bstar[j]==1) ) >= 1)
